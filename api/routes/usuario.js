    ...
    .then(found => {
      ...
      found
        ...
        .then(updated => {
          return res
            .status(200)
            .send('Credenciales reestablecidas correctamente')
        })
        .catch(err => {
          console.log(err)
        ...
        .spread((usuario, created) => {
          if (!created) {
            ...
            return res.status(200).send({
              msg: 'Ya existe un usuario con el mismo número de documento'
            })
          }

          let listaUsuarioRol = listaRoles.map(rol => ({
            rol_id: rol,
          ...
          for (let ur in listaUsuarioRol) {
            models.usuariorol
              .create(listaUsuarioRol[ur], { transaction: t })
              .then(usuarioRolInserted => {
