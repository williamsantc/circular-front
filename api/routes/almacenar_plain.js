...
router.get('/list', function (req, res, next) {
  ...
  options.include = [
    { model: models.circular, as: 'circular' }

  ]

  models.almacenar.findAll(options).then(objects => {
...
router.get('/get', function (req, res, next) {
  if(!req.query.circular) {
    ...
    models.almacenar.findOne({
      ...
      res.download('./uploads/' + object.alma_file, 'circular_download.pdf', (err) => {
        if (err) {
          console.log('ERR: ' + err)
          return
        }
