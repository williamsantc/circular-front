...
router.post('/gestionar', (req, res, next) => {
  ...
  if (!responsable.resp_id) {
    models.responsable
      .findOrCreate({
        where: { resp_nombre: responsable.resp_nombre },
        defaults: responsable
      })
      ...
      .then(created => {
        let msg = created
          ...
          : 'Ya existe un responsable con el mismo nombre'
        res.status(200).send({ msg: msg, processOk: created })
      })
  } else {
    models.responsable
      .update(responsable, { where: { resp_id: responsable.resp_id } })
      .then(responsableUpdated => {
        res.status(200).send({
          msg: 'Cambios guardados en el responsable correctamente',
          processOk: true
        })
      })
      .catch(error => {
        res
          .status(500)
          .send({ msg: 'Error guandando cambios en el responsable' })
        console.log(error)
      })
  }
})
