...
router.post('/gestionar', (req, res, next) => {
  ...
  if (!area.area_id) {
    models.area
      .findOrCreate({
        where: { area_nombre: area.area_nombre },
        defaults: area
      })
      .spread((area, created) => {
        return created
      ...
      .then(created => {
        let msg = created
          ...
          : 'Ya existe un area con el mismo nombre'
        res.status(200).send({ msg: msg, processOk: created })
      })
  } else {
    models.area
      .update(area, { where: { area_id: area.area_id } })
      .then(areaUpdated => {
        res.status(200).send({
          msg: 'Cambios guardados en el area correctamente',
          processOk: true
        })
      })
      .catch(error => {
