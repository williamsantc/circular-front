  ...
  models.usuario
    ...
    .then(userAccount => {
      if (userAccount) {
        return res
          .status(500)
          .send('El nombre de usuario que ingresó ya existe.')
      }
      ...
      models.usuario
        ...
        .then(userFound => {
          ...
          let randomSalt = CryptoJS.SHA512(randomWord(10).trim()).toString(
            CryptoJS.enc.Hex
          )
          userFound.usua_salt = randomSalt

          let newPassword = CryptoJS.SHA512(
            user.usua_password + randomSalt
