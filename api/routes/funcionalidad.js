...
router.post('/gestionar', (req, res, next) => {
  ...
  if (!funcionalidad.func_id) {
    models.funcionalidad
      .findOrCreate({ where: { func_descripcion: funcionalidad.func_descripcion }, defaults: funcionalidad })
      .spread((funcionalidad, created) => {
        return created
      }).then(created => {
        let msg = (funcionalidad ? 'Funcionalidad registrada correctamente' : 'Ya existe una funcionalidad con el mismo nombre')
        res.status(200).send({ msg: msg, processOk: created })
      })

  } else {
    models.funcionalidad.update(funcionalidad,
      { where: { func_id: funcionalidad.func_id } }).then(funcionalidadUpdated => {
        res.status(200).send({ msg: 'Cambios guardados en la funcionalidad correctamente', processOk: true })
      }).catch(error => {
        res.status(500).send({ msg: 'Error guandando cambios en la funcionalidad' })
        console.log(error)
      })
  }
});
